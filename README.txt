...
userId(Long) - идентификатор пользователя, path(String
) - путь страницы
Например, http://localhost:8080/rest/record?user=3&path=testPath
2) /rest/statistic?from={from}&to={to} - Получение статистики посещения за произвольный период. 
from и to - дата в формате dd.MM.yyyy
Параметры 
можно как указать все, так и указать по одному или ни одного.
Например,
